use std::collections::HashSet;

use crate::clauses::{Clause, Formula, Literal};

fn get_unit_clause(clauses: &Formula) -> Option<Literal> {
    clauses.iter().find(|c| c.len() == 1).map(|c| c[0])
}

fn unit_propogate(clauses: &mut Formula, lit: Literal) {
    clauses.retain(|c| !c.contains(&lit));

    for clause in clauses {
        clause.retain(|&l| l != -lit);
    }
}

fn get_pure_literals(clauses: &Formula) -> Vec<Literal> {
    let mut positive_literals = HashSet::new();
    let mut negative_literals = HashSet::new();

    for clause in clauses {
        for literal in clause {
            if *literal < 0 {
                negative_literals.insert(-literal);
            } else {
                positive_literals.insert(*literal);
            }
        }
    }

    let pure_positive = positive_literals
        .difference(&negative_literals)
        .copied()
        .collect::<Vec<Literal>>();

    let pure_negative = negative_literals
        .difference(&positive_literals)
        .map(|l| -(*l))
        .collect::<Vec<Literal>>();

    pure_positive.into_iter().chain(pure_negative).collect()
}

fn assign_pure_literals(clauses: &mut Formula, pure_literals: &[Literal]) {
    clauses.retain(|c| !pure_literals.iter().any(|&lit| c.contains(&lit)));
}

fn branch(clauses: &Formula, lit: Literal) -> Option<Vec<Literal>> {
    let mut new_clauses = clauses.clone();
    new_clauses.push(Clause::from([lit]));
    dpll(&mut new_clauses)
}

pub fn dpll(clauses: &mut Formula) -> Option<Vec<Literal>> {
    let mut true_literals: Vec<Literal> = Vec::new();

    while let Some(lit) = get_unit_clause(clauses) {
        unit_propogate(clauses, lit);
        true_literals.push(lit);
    }

    let pure_literals = get_pure_literals(clauses);
    assign_pure_literals(clauses, &pure_literals);
    true_literals.extend(pure_literals);

    if clauses.is_empty() {
        return Some(true_literals);
    }

    if clauses.iter().any(|c| c.is_empty()) {
        return None;
    }

    let lit = clauses[0][0];

    branch(clauses, lit)
        .or_else(|| branch(clauses, -lit))
        .map(|mut res| {
            res.extend(true_literals);
            res
        })
}

pub fn check_assignment(clauses: &Formula, assignment: &[Literal]) -> bool {
    // Note: assignemnt could be turned into a HashSet for performance later.
    clauses
        .iter()
        .all(|clause| clause.iter().any(|lit| assignment.contains(lit)))
}

#[cfg(test)]
mod tests {
    use super::*;

    #[test]
    fn unit_clause_some() {
        let clauses: Formula = vec![vec![1, 2], vec![1], vec![2, 3]];
        let unit_clause = get_unit_clause(&clauses);
        assert!(unit_clause.is_some());
    }

    #[test]
    fn unit_clause_none() {
        let clauses: Formula = vec![vec![1, 2], vec![1, 3], vec![2, 3]];
        let unit_clause = get_unit_clause(&clauses);
        assert!(unit_clause.is_none());
    }

    #[test]
    fn does_unit_propogation() {
        let mut clauses: Formula = vec![vec![1, 2], vec![1], vec![2, 3]];
        unit_propogate(&mut clauses, 1);
        assert_eq!(clauses, vec![vec![2, 3]]);
    }

    #[test]
    fn does_unit_propogation_negation() {
        let mut clauses: Formula = vec![vec![1, 2], vec![1], vec![2, 3], vec![2, -1]];
        unit_propogate(&mut clauses, 1);
        assert_eq!(clauses, vec![vec![2, 3], vec![2]]);
    }

    #[test]
    fn gets_pure_literals() {
        let clauses: Formula = vec![vec![1, 2], vec![1], vec![2, 3], vec![2, -1]];
        let pure_literals: HashSet<i64> = HashSet::from_iter(get_pure_literals(&clauses));
        assert_eq!(pure_literals, HashSet::from_iter(vec![2, 3]));
    }

    #[test]
    fn assigns_pure_literals() {
        let mut clauses: Formula = vec![vec![1, 2], vec![1], vec![2, 3], vec![2, -1]];
        assign_pure_literals(&mut clauses, &vec![2, 3]);
        assert_eq!(clauses, vec![vec![1]]);
    }

    #[test]
    fn check_assignment_true() {
        let clauses: Formula = vec![vec![1, 2], vec![1], vec![2, 3]];
        let assignment: Vec<Literal> = vec![1, 3];
        assert!(check_assignment(&clauses, &assignment));
    }

    #[test]
    fn check_assignment_false() {
        let clauses: Formula = vec![vec![1, 2], vec![1], vec![2, 3]];
        let assignment: Vec<Literal> = vec![2, 3];
        assert!(!check_assignment(&clauses, &assignment));
    }
}
